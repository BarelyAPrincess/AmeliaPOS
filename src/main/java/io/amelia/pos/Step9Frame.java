...
/**
 * Outline      The processing for printing on the slip is added.
 *
 * @author s.muroga
 * @version 1.00  (2001.05.07)
 */

public class Step9Frame extends JFrame implements OutputCompleteListener
{
	private POSPrinterControl114 ptr = ( POSPrinterControl114 ) new POSPrinter();
	...
	JButton jButton_PrintSalesSlip = new JButton();
	...
	private void jbInit() throws Exception
	{
		...
		titledBorder2 = new TitledBorder( border1, "Slip" );
		contentPane.setLayout( gridBagLayout1 );
		this.setSize( new Dimension( 300, 340 ) );
		this.setTitle( "Step 9 Print on slips." );
		...
		jPanel_Slip.setBorder( titledBorder2 );
		jPanel_Slip.setLayout( gridBagLayout3 );
		jButton_PrintSalesSlip.setText( "Print Sales Slip" );
		jButton_PrintSalesSlip.addActionListener( new ActionListener()
		{
			public void actionPerformed( ActionEvent e )
			{
				jButton_PrintSalesSlip_actionPerformed( e );
			}
		} );
		contentPane.add( jPanel_Receipt, new GridBagConstraints( 0, 0, 2, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets( 15, 0, 0, 0 ), 20, 20 ) );
		jPanel_Receipt.add( jButton_Print, new GridBagConstraints( 0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets( 5, 10, 5, 10 ), 130, 0 ) );
		jPanel_Receipt.add( jButton_AsynchronousPrinting, new GridBagConstraints( 0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets( 5, 10, 5, 10 ), 0, 0 ) );
		jPanel_Receipt.add( jButton_PrintReceipt, new GridBagConstraints( 0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets( 5, 10, 5, 10 ), 0, 0 ) );
		contentPane.add( jButton_Close, new GridBagConstraints( 0, 2, 2, 1, 0.0, 0.0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets( 10, 0, 0, 0 ), 0, 0 ) );
		contentPane.add( jPanel_Slip, new GridBagConstraints( 0, 1, 2, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets( 0, 0, 0, 0 ), 20, 20 ) );
		jPanel_Slip.add( jButton_PrintSalesSlip, new GridBagConstraints( 0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets( 5, 10, 5, 10 ), 72, 0 ) );
	}
	...
	protected void processWindowEvent( WindowEvent e )
	{
		...
		else if ( e.getID() == WindowEvent.WINDOW_OPENED )
		{
			...
			try
			{
					...
					if ( ptr.getCapRecBitmap() == true )
						...
						if ( ptr.getCapRecBitmap() == true )
						{
							...
							for ( int iRetryCount = 0; iRetryCount < 5; iRetryCount++ )
							{
								...
								{
									//Register a bitmap
									ptr.setBitmap( 1, POSPrinterConst.PTR_S_RECEIPT, "javapos.bmp", ( ptr.getRecLineWidth() / 2 ), POSPrinterConst.PTR_BM_CENTER );
				...
				// JavaPOS's code for Step9
				//Check on rotation print function
				if ( ( ptr.getCapSlpPresent() == false ) || ( ptr.getCapSlpFullslip() == false ) )
				{
					//Not function to [Print Receipt] button is disable
					this.jButton_PrintSalesSlip.setEnabled( false );
				}
				// JavaPOS's code for Step9--END
			}
			catch ( JposException ex )
			{
			}
	...
	public void outputCompleteOccurred( OutputCompleteEvent outComE )
	{
		...
		JOptionPane.showMessageDialog( this, "Complete printing : ID = " + stOutputCompleteEventText, "AsyncPrint", JOptionPane.PLAIN_MESSAGE );
	}
	// JavaPOS's code for Step7--END
	...
	void jButton_Print_actionPerformed( ActionEvent e )
	{
		...
		try
		{
			...
			if ( ptr.getCapRecPresent() == true )
			{
				...
				printData = makePrintString( ptr.getRecLineChars(), "Tax excluded.", formatForDouble( total ) );
				ptr.printNormal( POSPrinterConst.PTR_S_RECEIPT, "\u001b|bC" + printData + "\n" );

				printData = makePrintString( ptr.getRecLineChars(), "Tax  5.0%", formatForDouble( total * 0.05 ) );
				ptr.printNormal( POSPrinterConst.PTR_S_RECEIPT, "\u001b|uC" + printData + "\n" );

				printData = makePrintString( ( ptr.getRecLineChars() / 2 ), "Total", formatForDouble( total * 1.05 ) );
				...
				printData = makePrintString( ptr.getRecLineChars(), "Change", formatForDouble( 200.00 - ( total * 1.05 ) ) );
				ptr.printNormal( POSPrinterConst.PTR_S_RECEIPT, printData + "\n" );
				...
				if ( ptr.getCapRecBarCode() == true )
					ptr.printBarCode( POSPrinterConst.PTR_S_RECEIPT, bcData, POSPrinterConst.PTR_BCS_JAN13, 1000, ptr.getRecLineWidth(), POSPrinterConst.PTR_BC_CENTER, POSPrinterConst.PTR_BC_TEXT_BELOW );
				// JavaPOS's code for Step5--END
	...
	/**
	 * Outline      A method "PrintSalesSlip" calls some another method.
	 * They are method for printing and for inserting and
	 * removing the slip paper.
	 */
	void jButton_PrintSalesSlip_actionPerformed( ActionEvent e )
	{
		DateFormat df = DateFormat.getDateInstance();
		Time t = new Time( System.currentTimeMillis() );
		...
		try
		{
			...
			while ( true )
			{
				selectedValue = JOptionPane.showConfirmDialog( this, "Please insert a form.", "Insert", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE );
			...
			// Print data
			sendData = "\n" + leftSpace + "Print credit card sales slip\n";
			sendData = sendData + "\u001b|1lF";
			sendData = sendData + leftSpace + "        SEIKO EPSON Corp.\n";
			sendData = sendData + leftSpace + "Thank you for coming to our shop!\n";
			sendData = sendData + "\u001b|1lF";
			sendData = sendData + leftSpace + "Date " + date + "\n";
			sendData = sendData + leftSpace + "Time      " + time + "Casher   " + casher + "\n";
			sendData = sendData + leftSpace + "Number of the register" + regNo + "\n";
			sendData = sendData + "\u001b|N\u001b|1lF";
			sendData = sendData + leftSpace + "Details                      cost\n";
			sendData = sendData + leftSpace + "Cardigan                 $ 100.00\n";
			sendData = sendData + leftSpace + "Shoes                     $ 70.00\n";
			sendData = sendData + leftSpace + "Hat                       $ 30.00\n";
			sendData = sendData + leftSpace + "Bag                      $ 150.00\n";
			sendData = sendData + leftSpace + "        Excluded tax     $ 350.00\n";
			sendData = sendData + leftSpace + "        Tax(5%)           $ 17.50\n";
			sendData = sendData + leftSpace + "        -------------------------\n";
			sendData = sendData + leftSpace + "\u001b|2C     Total\u001b|1C     $ 367.50\n";
			sendData = sendData + "\u001b|1lF";
			sendData = sendData + leftSpace + "Company name   EPSON-CARD\n";
			sendData = sendData + leftSpace + "Membership No. XXXXXXXXXXXXXXXX\n";
			sendData = sendData + leftSpace + "Valid date     12/05\n";
			sendData = sendData + leftSpace + "Handling No.   9999 - 999999\n";
			sendData = sendData + leftSpace + "Approval No.   99\n";
			sendData = sendData + "\u001b|1lF";
			sendData = sendData + leftSpace + "Signature\n";
			//Printing process
			ptr.printNormal( POSPrinterConst.PTR_S_SLIP, sendData );

			//Remove the slip at the slip station.
			ptr.beginRemoval( 10000 );
			ptr.endRemoval();
		}
		catch ( JposException ex )
		{
	 ...
	 *
	 * @param contents There is "beforeAddingTax" "tax" "total"
	 *                 "payment" "change" as a value which "contents" holds.
	 *
